...
add_action('wp_headers', function() {
  ...
  $csp = $csp . implode('; ', array_filter([
    (defined('WP_HEADERS_CSP_DEFAULT'))
      ? "default-src 'self' " . WP_HEADERS_CSP_DEFAULT : "default-src 'self'",
    (defined('WP_HEADERS_CSP_STYLE'))
      ? "style-src 'self' " . WP_HEADERS_CSP_STYLE : "style-src 'self'",
    (defined('WP_HEADERS_CSP_FONT'))
      ? "font-src 'self' " . WP_HEADERS_CSP_FONT : "font-src 'self'",
    (defined('WP_HEADERS_CSP_IMG'))
      ? "img-src 'self' " . WP_HEADERS_CSP_IMG : "img-src 'self'",
    (defined('WP_HEADERS_CSP_SCRIPT'))
      ? "script-src 'self' 'nonce-$scripts' " . WP_HEADERS_CSP_SCRIPT : "script-src 'self' 'nonce-$scripts'",
    (defined('WP_HEADERS_CSP_CONNECT'))
      ? "connect-src 'self' " . WP_HEADERS_CSP_CONNECT : "connect-src 'self'",
    (defined('WP_HEADERS_CSP_FRAME'))
      ? "frame-src " . WP_HEADERS_CSP_FRAME : "frame-src 'none'",
    (defined('WP_HEADERS_CSP_OBJECT'))
      ? "object-src " . WP_HEADERS_CSP_OBJECT : "object-src 'none'"
  ]));

  // Append the reporting URL if needed
  if (defined('WP_HEADERS_CSP_REPORTING') && WP_HEADERS_CSP_REPORTING) {
    $csp = $csp . '; report-uri ' . WP_HEADERS_CSP_REPORTING . '; report-to ' . WP_HEADERS_CSP_REPORTING;
  }
